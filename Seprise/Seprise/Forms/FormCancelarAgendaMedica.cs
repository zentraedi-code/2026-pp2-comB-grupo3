...
namespace Seprise
{
    public partial class FormCancelarAgendaMedica : Form
    {
        ...
        private void ConfigurarGrilla()
        {
            ...
            dgvAgendas.ColumnHeadersDefaultCellStyle.SelectionBackColor = Color.FromArgb(0, 150, 136);
            dgvAgendas.ColumnHeadersDefaultCellStyle.SelectionForeColor = Color.White;
            dgvAgendas.DefaultCellStyle.SelectionBackColor = Color.FromArgb(120, 200, 190);
            dgvAgendas.DefaultCellStyle.SelectionForeColor = Color.Black;
            ...
            dgvAgendas.Columns.Add(new DataGridViewTextBoxColumn { Name = "colId", HeaderText = "ID", Visible = false });
        ...
        private void btnBuscar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataTable dt = dao.listarActivasPorFiltro(fecha, medicoId);

                dgvAgendas.Rows.Clear();

                if (dt == null || dt.Rows.Count == 0)
                {
                    MessageBox.Show("No se encontraron agendas ACTIVAS con los filtros indicados.", "Sin resultados", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
        ...
        {
            if (e.RowIndex < 0 || e.ColumnIndex != dgvAgendas.Columns["colCancelar"].Index)
                return;

            int agendaId = (int)dgvAgendas.Rows[e.RowIndex].Tag;

            var respuesta = MessageBox.Show(
                "¿Cancelar esta agenda? Se eliminarán todos los turnos asociados.",
                "Cancelar agenda",
            ...
            {
                AgendaMedicaDao dao = new AgendaMedicaDao();
                bool ok = dao.cancelarConTurnos(agendaId);

                if (ok)
                {
                    MessageBox.Show("Agenda cancelada y turnos eliminados correctamente.", "Éxito", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    dgvAgendas.Rows.RemoveAt(e.RowIndex);
                }
                else
                {
                    MessageBox.Show("No se pudo cancelar la agenda.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error al cancelar la agenda: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
